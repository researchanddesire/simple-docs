...
The Trainer can reach the internet through your mobile connection.

</Callout>

## Permanent router configuration

Once you've confirmed the router is the issue, you can create a permanent fix one of two ways;

<Accordions type="single">
  <Accordion title="Create an allow rule for the Trainer">
...
Find the MAC address in your Trainer's network settings or on the device label.
  ...
  <Accordion title="Adjust IoT device settings">

Some routers have specific IoT device settings. Look for options like:

1. **IoT Network** — Add the Trainer to a trusted IoT network
2. **Device Priority** — Set the Trainer to high priority
3. **Access Scheduling** — Ensure no time-based restrictions apply
