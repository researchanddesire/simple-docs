...
export const GoldMotorControl = () => {
  ...
  const NumberInput = ({ label, regLabel, id, disabled = false, min, max }) => (
    <div className="flex flex-col">
      ...
      <span className="mb-1 text-xs text-zinc-500 dark:text-zinc-500">
        {regLabel}
        {disabled && (
      ...
      <input
        ...
        min={min}
        max={max}
        className="rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 disabled:bg-zinc-100 disabled:text-zinc-500 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100 dark:disabled:bg-zinc-900"
      />
    </div>
  );
    ...
    <div className="flex flex-col">
      ...
      <button
        ...
        className={`relative h-7 w-14 rounded-full transition-colors ${
          formValues[id] ? "bg-violet-500" : "bg-zinc-300 dark:bg-zinc-600"
        }`}
      >
        <span
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-2xl">
        ...
        {!isConnected && (
          <div className="mb-4 rounded-lg border border-zinc-300 bg-zinc-100 p-4 dark:border-zinc-600 dark:bg-zinc-800">
            <p className="mb-2 font-semibold text-zinc-800 dark:text-zinc-200">
              Connection Instructions:
            </p>
            <p className="mb-2 text-sm text-zinc-600 dark:text-zinc-400">
              Connect the motor using an <strong>RS485 USB adapter</strong> that
              provides 5V+, GND, A, B.{" "}
              <a
        ...
        <div className="mb-4 flex justify-center gap-4">
          {!isConnected ? (
            <button
              ...
              className="rounded-full bg-violet-500 px-6 py-2 font-medium text-white transition-colors hover:bg-violet-600"
            >
              Connect to Motor
            </button>
          ) : (
            <button
              ...
              className="rounded-full bg-zinc-500 px-6 py-2 font-medium text-white transition-colors hover:bg-zinc-600"
            >
              Disconnect
            </button>
          )}
        ...
        {isConnected && (
          <div>
            <h3 className="text-md mb-3 font-semibold text-zinc-800 dark:text-zinc-200">
              Motor Settings
            </h3>

            <button
              onClick={resetDefaults}
              className="mb-4 rounded-lg bg-zinc-200 px-4 py-1.5 text-sm text-zinc-700 transition-colors hover:bg-zinc-300 dark:bg-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-600"
            >
            ...
            {registerOutput.length > 0 && (
              <div className="mb-4">
                <h3 className="text-md mb-2 font-semibold text-zinc-800 dark:text-zinc-200">
                  Register Values
                </h3>
                <div className="max-h-48 overflow-auto rounded-lg bg-zinc-900 p-3 font-mono text-xs dark:bg-black">
                  {registerOutput.map((reg, idx) => (
                    <div
                      key={idx}
                      className={
                        reg.highlighted ? "text-green-400" : "text-zinc-300"
                      }
                    >
          ...
          <div
            className={`rounded-lg p-3 text-sm ${
              feedback.type === "success"
                ? "border border-green-300 bg-green-100 text-green-800 dark:border-green-700 dark:bg-green-900/30 dark:text-green-300"
                : feedback.type === "error"
                  ? "border border-red-300 bg-red-100 text-red-800 dark:border-red-700 dark:bg-red-900/30 dark:text-red-300"
                  : "border border-zinc-300 bg-zinc-100 text-zinc-700 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300"
            }`}
          >
            {feedback.message}
          </div>
