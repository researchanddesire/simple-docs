...
<AccordionGroup>
  ??? note "What comes with my Deepthroat Trainer order?"
...
<AccordionGroup>
  ??? note "Can I use the Trainer handheld instead of wall-mounted?"
    ...
    <Card title="Mounting Guide" icon="screwdriver" href="/dtt/quick-start/getting-started">
      Step-by-step instructions for mounting your Trainer.
    </Card>
  
??? note "Can I wear the Trainer so a partner can practice on me?"
...
??? note "I don't have a suitable mounting surface. What can I do?"
...
<AccordionGroup>
  ??? note "What toys can I use with my Deepthroat Trainer?"
    ...
    <Card title="Using Different Toys" icon="box" href="/dtt/quick-start/using-toys">
      How to set your default toy, add your own, and assign toys to templates.
    </Card>
  
??? note "Can I use the vibrating plug and wand at the same time?"
    ...
    **How to use the wand:** The wand is designed for external stimulation. Hold it in place manually, tuck it into underwear, or position it however works best for you.
...
<Card title="Accessory Port" icon="plug" href="/dtt/specs/accessory-port">
...
??? note "What size vibrating plugs are available?"
...
??? note "Can I use my own dildo with the Deepthroat Trainer?"
    ...
    <Card title="Add or select your toy" icon="settings" href="/dtt/quick-start/using-toys">
      Step‑by‑step Toybox instructions.
    </Card>
  
??? note "What is the maximum length the Deepthroat Trainer can sense?"
...
<AccordionGroup>
  ??? note "How do I recalibrate my toy's depth setting?"
    ...
    <Card title="Calibration Guide" icon="sliders" href="/dtt/technical/calibration#how-to-reset-calibration-for-a-toy">
      Step-by-step reset and recalibration instructions.
    </Card>
  
??? note "Can I recalibrate my toy mid-session?"

    No. Calibration changes cannot take effect during an active session. The Trainer fetches your calibration data when you start a new session, so changes made in the dashboard won't apply until the next session.
...
??? note "Training requires depths that are physically impossible to reach"
    ...
    <Card title="Calibration Troubleshooting" icon="sliders" href="/dtt/technical/calibration#training-requires-depths-i-cant-physically-reach">
...
<AccordionGroup>
  ??? note "How does the Deepthroat Trainer track my performance?"
...
??? note "What are Training Points and how are they calculated?"

    Training Points measure your session performance. You earn points from depth, duration, rhythm, and consistency across modes.

    <Card title="Learn about Points" icon="calculator" href="https://dashboard.researchanddesire.com/leaderboards/points">
      See the complete breakdown of how Training Points are calculated.
    </Card>
  
??? note "How does the leaderboard work?"

    Leaderboards rank you based on accumulated Training Points, with weekly, monthly, and all‑time views.

    <Card title="Leaderboard Guide" icon="trophy" href="https://dashboard.researchanddesire.com/leaderboards">
...
<AccordionGroup>
  ??? note "My multi-segment template only plays the first segment"
...
??? note "Do I need to restart the trainer to switch templates?"
...
??? note "Template editor stuck on 'loading' when saving"
...
??? note "Templates not syncing — device runs default/demo mode instead"

    If your trainer is connected to Wi-Fi but runs a default mode (such as 1200 rpm freeform) or shows built-in demo programs instead of your custom templates, the device isn't properly receiving your template settings.
    ...
    **If the problem started after a firmware update:**

    Occasionally, a firmware update may complete but leave the device in a state where it can't properly sync template data from the server. If you've tried all the above steps:
    ...
    2. **Contact support** — if re-pairing doesn't resolve the issue, contact [support@researchanddesire.com](mailto:support@researchanddesire.com) with your Trainer ID and order number. Support can check your account sync status and may need to reset the device's server-side state.
...
<AccordionGroup>
  ??? note "Wi-Fi shows connected but device says offline"
...
??? note "Trainer stuck on 'Getting your settings'"
    ...
    **If the issue is related to templates:**
    ...
    Sometimes the device can get stuck if template data becomes corrupted or an incomplete download occurred (for example, if a previous session was interrupted before the template finished loading).
    ...
    **If the issue started after a firmware update:**
...
??? note "Training won't start or the device shows a message about waiting"
    ...
    **Quick fixes:**
    ...
    <CardGroup cols={2}>
      <Card title="Wi‑Fi Troubleshooting" icon="wifi" href="/dtt/quick-start/pairing/wifi-setup">
...
??? note "Can I use my Trainer offline?"
...
<Card title="Software 2.0.0 Beta Preview" icon="lab" href="/dtt/software-2.0-preview">
      What’s new now and what’s coming next.
    </Card>
  
??? note "How do I know if my firmware is up to date?"

    When your Trainer is connected to Wi‑Fi, it automatically checks for available updates each time you power it on. If an update is available, you'll see an "Updating" screen with progress information—the device downloads and installs the new firmware automatically, then restarts.
...
??? note "Is my older Trainer eligible for firmware updates?"
...
<Card title="Step‑by‑step Wi‑Fi guide" icon="list" href="/dtt/quick-start/pairing/wifi-setup">
      Screen‑by‑screen instructions.
    </Card>
  
??? note "How can I tell if my Trainer is connected to Wi‑Fi?"

    When you power on the device, the display shows the current Wi‑Fi connection status in plain text.
  
??? note "Wi-Fi doesn't stay connected after restarting the device"

    If your trainer connects to Wi-Fi during setup but shows "no wifi connection" or requires manual reconnection every time you power cycle:

    **Common causes:**

    - **Network credentials not saving properly** — the Wi-Fi password may not have been stored correctly
    - **Router compatibility issues** — some routers have settings that prevent reconnection
    - **Firmware issue** — older firmware versions may have Wi-Fi persistence bugs

    **Quick fixes:**

    1. **Re-run Wi-Fi setup completely** — long-press during startup to enter Wi-Fi setup, then re-enter your network credentials from scratch
    2. **Verify 2.4 GHz network** — ensure you're connecting to a 2.4 GHz network, not 5 GHz
    3. **Try a different network** — test with a mobile hotspot to rule out router-specific issues
    4. **Check for firmware updates** — connect to any working network first to receive updates, then reconfigure your home Wi-Fi

    **If the problem persists:**

    - Check if your router has MAC address filtering or device isolation enabled
    - Some mesh networks or ISP-provided routers have aggressive power-saving that drops inactive connections
    - Try assigning a static IP to your trainer in your router settings (use the trainer's MAC address)

    !!! warning
If your trainer requires manual reconnection on every boot despite correct configuration, contact [support@researchanddesire.com](mailto:support@researchanddesire.com) with your Trainer ID and order number. This may indicate a hardware or firmware issue requiring further diagnosis.
...
<AccordionGroup>
  ??? note "How do I clean my Deepthroat Trainer?"
...
??? note "How do I clean and care for my dildo?"
...
??? note "I accidentally used silicone lube on my dildo. Should I throw it out?"
...
<AccordionGroup>
  ??? note "Why are so many features marked as Ultra-only?"
    ...
    !!! tip
If you're unsure whether a specific feature will eventually become free, email [support@researchanddesire.com](mailto:support@researchanddesire.com) or ask in our Discord community.

      
<Card title="Subscription Plans" icon="gem" href="https://dashboard.researchanddesire.com/subscription">
      Compare tiers and see what's included.
    </Card>
  
??? note "How does the AI Training Schedule work?"
...
??? note "How do I subscribe to Ultra?"
...
<Card title="View Subscription Options" icon="credit-card" href="https://dashboard.researchanddesire.com/subscription">
      See all tiers and benefits.
    </Card>
  
??? note "What is the segment repeat feature?"
...
??? note "Does my keyholder need Ultra to create advanced templates for me?"
...
??? note "I get an error when adding more than 3 segments to my partner's template"
...
??? note "Can I give my keyholder control of the Trainer but not the Lockbox (or vice versa)?"
...
<Card title="Partner Connections" icon="users" href="https://dashboard.researchanddesire.com/quick-start/connections">
...
<AccordionGroup>
  ??? note "How do I sell or donate my Deepthroat Trainer?"

    If you want to pass your Trainer to someone else:

    
### Step 3: Unpair your device
...
!!! info
Once unpaired, the next person to pair the device becomes the new owner. See the <a href="https://dashboard.researchanddesire.com/faqs/ownership-disputes">
      device ownership policy</a>
...
<AccordionGroup>
??? note "I can't access my dashboard account. How do I unpair my device?"
...
<Card title="Device Ownership Policy" icon="shield" href="https://dashboard.researchanddesire.com/faqs/ownership-disputes">
...
<AccordionGroup>
  ??? note "Toybox and template buttons are greyed out"

    If you can't edit toys, create templates, or modify your training settings—all buttons appear disabled or greyed out—you likely have **Give Up Control** enabled in your account settings.

    **What's happening:**

    When Give Up Control is enabled, you transfer control of your devices to your assigned Keyholder. This disables your ability to:

    - Edit or add toys in your Toybox
    - Create or modify templates
    - Change training settings

    Only your Keyholder can make these changes while Give Up Control is active.

    **How to fix:**

    1. Go to **Settings → Advanced** in your dashboard
    2. Toggle off **Give Up Control**
    3. If you selected "Delayed" consent when enabling it, you'll need to wait 72 hours before control is restored
    4. If you selected "Permanent" consent, only your Keyholder can return control to you

    !!! tip
If you don't remember enabling Give Up Control, check with your partner—they may have enabled it on your behalf, or you may have enabled it during initial setup.
    
<Card title="Understanding Access Types" icon="users" href="/dashboard/quick-start/connections/access-types">
      Learn more about Give Up Control and partner access settings.
    </Card>
  
??? note "Can't find the settings menu on my device"
    ...
    **What "settings" means on the device:**
    ...
    **How to configure your trainer:**
    ...
    1. Sign in to your dashboard at [dashboard.researchanddesire.com](https://dashboard.researchanddesire.com)
    ...
    **Common confusion:**
    ...
    <CardGroup cols={2}>
      <Card title="Button Controls" icon="hand-pointer" href="/dtt/technical/button-controls">
        Complete guide to button actions by screen.
      </Card>
      <Card title="Templates" icon="file-lines" href="/dtt/training-modes/introduction">
        Learn how to create and manage training templates.
      </Card>
    </CardGroup>
  
??? note "LED lights up but the screen doesn't turn on"
...
??? note "Device won't turn on at all"
...
??? note "Can I replace the battery in my Trainer?"

    Battery replacement is possible but not straightforward. The battery is glued in place inside the enclosure, so removal requires care and some technical skill.
    ...
    **Replacement battery options:**
    ...
    We custom-order batteries from our manufacturer with specific polarity and wiring. Contact [support@researchanddesire.com](mailto:support@researchanddesire.com) to purchase a replacement battery (approximately $8 USD plus shipping).
...
??? note "Accessory port only works at certain angles or has intermittent connection"
    ...
    **Before contacting support, try these steps:**
    ...
    2. **Clean the port** — use compressed air to remove any debris from inside the port
    3. **Test a different accessory** — if you have another 3.5mm accessory, check if the issue persists

    **If the problem continues after these steps:**

    The port itself likely has a loose internal connection or damaged solder joint. This is a hardware defect that cannot be fixed through troubleshooting.

    Contact [support@researchanddesire.com](mailto:support@researchanddesire.com) with:

    - Your order number
    - A description of the issue (when it started, what you've tried)

    Support can discuss replacement options for your device.

    !!! warning
Do not attempt to open the device or repair the port yourself—this will void any warranty coverage.
    
<Card title="Accessory Port Specs" icon="plug" href="/dtt/specs/accessory-port">
...
<AccordionGroup>
  ??? note "Is there an open API for the Deepthroat Trainer?"

    There is no public API specification for the Deepthroat Trainer, OSSM, or dashboard at this time. The devices communicate with R+D servers over proprietary protocols, and there is no documented way to integrate them with third-party applications or AI agents.

    This is something we're considering for the future, but there is no timeline for availability.

    !!! tip
If developer access is important to you, let us know at [support@researchanddesire.com](mailto:support@researchanddesire.com)—user feedback helps prioritize the roadmap.
...
<AccordionGroup>
  ??? note "Is the Deepthroat Trainer compatible with OSSM?"
...
??? note "How do I find my Trainer ID?"

    Your five‑character alphanumeric Trainer ID displays on screen during startup.
  
??? note "My Trainer ID shows jumbled characters or changes on every reboot"
...
??? note "I have questions not covered here. Who should I contact?"

    For additional support, contact our team:

    - Email: [support@researchanddesire.com](mailto:support@researchanddesire.com)
  
??? note "How long does the battery last, and how long does it take to charge?"
